...
const generateMultiSigScript = async (privkeyCount, pubkeys, type = 'segwit') => {
    ...
    if (type !== 'taproot') {
        ...
        let privkeyCountHex = privkeyCount.toString(16);
        privkeyCountHex = await (0, encode_js_1.padZeroHexN)(privkeyCountHex, privkeyCountHex.length < 3 ? 2 : 4);
        // multi sig type of tapscript(OP_CHECKSIGADD)
        multiSigScript +=
            (await (0, data_js_1.pushData)(privkeyCountHex)) +
                privkeyCountHex +
                opcode_js_1.Opcode.OP_GREATERTHANOREQUAL;
    }
...
const generateTimeLockScript = async (block) => {
    ...
    return (await (0, data_js_1.pushData)(locktime)) + locktime + opcode + opcode_js_1.Opcode.OP_DROP;
};
