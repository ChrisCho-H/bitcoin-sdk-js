...
const getTapTweak = async (schnorrPubkey, taproot) => {
    await validator_js_1.Validator.validateKeyPair(schnorrPubkey, '', 'schnorr');
    ...
    return await (0, crypto_js_1.sha256)(!taproot ? tweakPubOnly : new Uint8Array([...tweakPubOnly, ...taproot]));
};
...
const getTapTweakedPrivkey = async (schnorrPrivkey, tapTweak) => {
    await validator_js_1.Validator.validateKeyPair('', schnorrPrivkey, 'schnorr');
...
const getTapControlBlock = async (schnorrPubkey, tweakedPubKeyParityBit, tapTreePath, tapLeafVersion = 0xc0) => {
    await validator_js_1.Validator.validateKeyPair(schnorrPubkey, '', 'schnorr');
    tapLeafVersion += tweakedPubKeyParityBit === '02' ? 0x00 : 0x01;
    return (tapLeafVersion.toString(16) +
        schnorrPubkey +
        (await (0, encode_js_1.bytesToHex)(tapTreePath)));
};
