...
class Transaction {
    ...
    _sign = async (pubkey, privkey, unsignedTx, inputIdx, isMultiSig, timeLockScript = '', secretHex = '', type = 'segwit', sigHashType = '01000000') => {
        ...
        if (!isMultiSig) {
            // p2pkh scrip sig
            const signature = await (0, crypto_js_1.sign)(msgHash, privkey[0], type !== 'taproot' ? 'secp256k1' : 'schnorr', sigHashType);
            scriptSig +=
            ...
            for (let i = 0; i < privkey.length; i++) {
                if (privkey[i].length !== 64)
                    throw new Error('privkey must be 32 bytes');
                const signature = await (0, crypto_js_1.sign)(msgHash, privkey[i], 'secp256k1', sigHashType);
