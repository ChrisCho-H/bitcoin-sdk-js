...
(0, mocha_1.describe)('sign and verify message test', () => {
    (0, mocha_1.it)('signature must be verified as true', async () => {
        // Given
        const keyPair = await bitcoin.wallet.generateKeyPair();
        ...
        const tapAddress = await bitcoin.address.generateAddress((await bitcoin.tapscript.getTapTweakedPubkey(pubkey.slice(2), await bitcoin.tapscript.getTapTweak(pubkey.slice(2)))).tweakedPubKey, 'taproot');
        const msg = await bitcoin.encode.bytesToHex(crypto.getRandomValues(new Uint8Array(32)));
        // When
        const sigLegacy = await bitcoin.crypto.signMessage(msg, privkey, legacyAddress);
        ...
        // Then
        assert.strictEqual(await bitcoin.crypto.verifyMessage(msg, sigLegacy, legacyAddress), true);
        assert.strictEqual(await bitcoin.crypto.verifyMessage(msg, sigSegwit, segwitAddress), true);
        assert.strictEqual(await bitcoin.crypto.verifyMessage(msg, sigTap, tapAddress), true);
    });
});
