...
import { bytesToHex, padZeroHexN, hexToBytes, utf8ToBytes, bytesToBase64, base64ToBytes, } from './encode.js';
...
export const sign = async (msgHash, privkey, type = 'ecdsa', sigHashType = '01000000') => {
    // for validation
...
export const verify = async (signature, msgHash, pubkey, type = 'ecdsa', sigHashType = '01000000') => {
    // for validation
...
export const signMessage = async (msg, privkey, address) => {
    ...
    if (script.slice(0, 2) === Opcode.OP_DUP) {
        ...
        const msgHash = await hash256(new Uint8Array([...prefix, ...(await hexToBytes(len)), ...msgHex]));
        const sig = secp256k1.sign(msgHash, privkey);
        return bytesToBase64(new Uint8Array([sig.recovery + 31, ...sig.toCompactRawBytes()]));
    }
    else {
        const pubkey = await bytesToHex(secp256k1.getPublicKey(privkey));
        ...
        if (script.slice(0, 2) === Opcode.OP_0) {
            ...
            const tapTweak = await getTapTweak(pubkey.slice(2));
            const tweakedPrivKey = await getTapTweakedPrivkey(privkey, tapTweak);
        ...
        return bytesToBase64(await hexToBytes(txToSign._witness.get(0)));
    }
};
export const verifyMessage = async (msg, signature, address) => {
...
const _getVirtualTx = async (msg, script) => {
    ...
    const msgHash = await bytesToHex(await sha256(new Uint8Array([
