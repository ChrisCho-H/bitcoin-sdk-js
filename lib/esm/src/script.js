...
export const generateMultiSigScript = async (privkeyCount, pubkeys, type = 'segwit') => {
    ...
    if (type !== 'taproot') {
        ...
        const privkeyCountHex = await scriptNum(privkeyCount);
        const dataToRead = privkeyCount <= 16 ? '' : await pushData(privkeyCountHex);
...
export const generateTimeLockScript = async (block) => {
    ...
    const locktime = await scriptNum(block);
    const dataToRead = block <= 16 ? '' : await pushData(locktime);
