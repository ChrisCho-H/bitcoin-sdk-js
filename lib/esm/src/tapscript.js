...
export const getTapBranch = async (tapLeafPair) => {
    ...
    const orderedTapPair = new Uint8Array(BigInt('0x' + (await bytesToHex(tapLeafPair[0]))) <
        BigInt('0x' + (await bytesToHex(tapLeafPair[1])))
        ? [...tapLeafPair[0], ...tapLeafPair[1]]
        : [...tapLeafPair[1], ...tapLeafPair[0]]);
    return await sha256(new Uint8Array([...(await getTapTag(tapBranchTagHex)), ...orderedTapPair]));
};
...
export const getTapTag = async (tapTagHex) => {
    const tapTagHash = await sha256(await hexToBytes(tapTagHex));
...
export const getTapSigHash = async (sigMsg) => {
    return await sha256(new Uint8Array([...(await getTapTag(tapSighashTagHex)), ...sigMsg]));
};
export const getTapControlBlock = async (schnorrPubkey, tapLeaf, tweakedPubKeyParityBit, tapLeafVersion = 0xc0) => {
    tapLeafVersion += tweakedPubKeyParityBit === '02' ? 0x00 : 0x01;
    return (tapLeafVersion.toString(16) + schnorrPubkey + (await bytesToHex(tapLeaf)));
};
