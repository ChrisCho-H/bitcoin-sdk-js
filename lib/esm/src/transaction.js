...
export class Transaction {
    ...
    signAll = async (pubkey, privkey, isSegWit = true) => {
        if (pubkey.length !== 66)
    ...
    signInput = async (pubkey, privkey, index, timeLockScript = '', secretHex = '', isSegWit = true) => {
        if (pubkey.length !== 66)
            throw new Error('pubkey must be compressed 33 bytes');
        if (privkey.length !== 64)
            throw new Error('privkey must be 32 bytes');
        const unsignedTx = await this._finalize();
        await this._sign([pubkey], [privkey], unsignedTx, index, false, timeLockScript, secretHex, isSegWit);
    };
    multiSignInput = async (pubkey, privkey, index, timeLockScript = '', secretHex = '', isSegWit = true) => {
        const unsignedTx = await this._finalize();
        await this._sign(pubkey, privkey, unsignedTx, index, true, timeLockScript, secretHex, isSegWit);
    };
    unlockHashInput = async (secretHex, index, timeLockScript = '', isSegWit = true) => {
    ...
    _finalizeSegwit = async () => {
        ...
        const prevHash = await hash256(hexToBytes(this._inputScriptArr
            .reduce((accumulator, currentValue) => accumulator + currentValue.slice(0, 72), '')
            .slice(2)));
        const sequenceHash = await hash256(hexToBytes(this._sequence.repeat(this._inputs.length)));
        this._witnessPrefix = new Uint8Array([
            ...versionByte,
            ...prevHash,
            ...sequenceHash,
        ]);
        const outputHash = await hash256(hexToBytes(this._outputScriptArr.join('').slice(2)));
    ...
    _sign = async (pubkey, privkey, unsignedTx, inputIdx, isMultiSig, timeLockScript = '', secretHex = '', isSegWit) => {
        ...
        if (!isMultiSig) {
            // p2pkh scrip sig
            const signature = secp256k1.sign(msgHash, privkey[0]).toDERHex() +
            ...
            for (let i = 0; i < privkey.length; i++) {
                if (privkey[i].length !== 64)
                    throw new Error('privkey must be 32 bytes');
                const signature = secp256k1.sign(msgHash, privkey[i]).toDERHex() +
                    sigHashType.slice(0, 2);
