...
describe('get varirable int test', () => {
  it('number converted to bitcoin varInt', async () => {
    ...
    // When
    const varInt1: string = await bitcoin.data.getVarInt(num1);
    const varInt2: string = await bitcoin.data.getVarInt(num2);
    const varInt3: string = await bitcoin.data.getVarInt(num3);
    const varInt4: string = await bitcoin.data.getVarInt(num4);
    ...
    assert.strictEqual(
      num2,
      parseInt(await bitcoin.encode.reverseHex(varInt2.slice(2)), 16),
    );
    ...
    assert.strictEqual(
      num3,
      parseInt(await bitcoin.encode.reverseHex(varInt3.slice(2)), 16),
    );
    ...
    assert.strictEqual(
      num4,
      parseInt(await bitcoin.encode.reverseHex(varInt4.slice(2)), 16),
    );
  });
});
...
describe('push data test', () => {
  it('push data encode for string', async () => {
    ...
    const num3 = Math.floor(
      Math.random() * (65535 * 2 - 256 * 2 + 1) + 256 * 2,
    );
    ...
    // When
    const pushData1: string = await bitcoin.data.pushData(str1);
    const pushData2: string = await bitcoin.data.pushData(str2);
    const pushData3: string = await bitcoin.data.pushData(str3);
    // Then
    assert.strictEqual(
      parseInt(pushData1, 16) * 2,
      num1 % 2 === 0 ? num1 : num1 + 1,
    );

    assert.strictEqual(pushData2.slice(0, 2), bitcoin.Opcode.OP_PUSHDATA1);
    assert.strictEqual(
      parseInt(pushData2.slice(2), 16) * 2,
      num2 % 2 === 0 ? num2 : num2 + 1,
    );

    assert.strictEqual(pushData3.slice(0, 2), bitcoin.Opcode.OP_PUSHDATA2);
    assert.strictEqual(
      parseInt(pushData3.slice(4, 6) + pushData3.slice(2, 4), 16) * 2,
      num3 % 2 === 0 ? num3 : num3 + 1,
    );
  });
});
