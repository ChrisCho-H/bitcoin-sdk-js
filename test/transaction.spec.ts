    ...
    await tx.signInputByScriptSig(
      [
        await bitcoin.crypto.sign(
          await tx.getInputHashToSign(HTLC, 3, 'legacy'),
          keypair.privateKey,
        ),
        secretHex,
        '',
...
describe('segwit transaction test', () => {
  it('p2pkh, p2wpkh, p2tr address must be generated', async () => {
    ...
    const index: number = Math.floor(Math.random() * (0xffffffff - 0 + 1) + 0);
    const value: number = Math.random() * (Number.MAX_SAFE_INTEGER - 0 + 1) + 0;
    const keypair: bitcoin.wallet.KeyPair =
      await bitcoin.wallet.generateKeyPair();
    const locktime: number = Math.floor(
    ...
    await tx.signInputByScriptSig(
      [
        await bitcoin.crypto.sign(
          await tx.getInputHashToSign(HTLC, 3, 'segwit'),
          keypair.privateKey,
        ),
        secretHex,
        '',
...
describe('taproot transaction test', () => {
  it('p2pkh, p2wpkh, p2tr address must be generated', async () => {
    ...
    await tx.signInputByScriptSig(
      [
        await bitcoin.crypto.sign(
          await tx.getInputHashToSign(HTLC, 3),
          keypair.privateKey,
        ),
        secretHex,
        '',
